...
namespace The_Simple_Game.Models
{
    public class Gameboard : ObservableObject
    {
        #region ENUMS

        public const string PLAYER_PIECE_1 = "▇";
        public const string PLAYER_PIECE_2 = "⬤";
        ...
        public void InitializeGameboard()
        {
            ...
            for (int row = 0; row < MAX_NUM_OF_ROWS_COLUMNS; row++)
            {
                for (int column = 0; column < MAX_NUM_OF_ROWS_COLUMNS; column++)
                {
                    ...
                    OnPropertyChanged(CurrentBoard[row][column]);
                }
            }
        }
        ...
        {
            if (FourInARow(PLAYER_PIECE_1))
            {
                CurrentRoundState = GameboardState.Player1Win;
            }
            else if (FourInARow(PLAYER_PIECE_2))
            {
                CurrentRoundState = GameboardState.Player2Win;
            }
            else if (IsCatsGame())
            {
                CurrentRoundState = GameboardState.NoWin;
            }
        }
        ...
        {
            if (CurrentRoundState == GameboardState.Player1Turn)
            {
                CurrentRoundState = GameboardState.Player2Turn;
            }
            else
            {
                CurrentRoundState = GameboardState.Player1Turn;
            }
        }
        #endregion
    }
